...
boxdb download <วันที่>[/<ไฟล์>] <โฟลเดอร์ปลายทาง>   # ดาวน์โหลดจาก S3
boxdb schedule    # ดู / ติดตั้ง / ถอน การอัพโหลดอัตโนมัติ
boxdb test-email  # ส่งอีเมลแจ้งเตือนตัวอย่าง
```
...
ดึงไฟล์เดียวหรือทั้งโฟลเดอร์วันที่กลับมาจาก S3:

```sh
boxdb download 2026-07-08/db1.pg /root/restore   # ไฟล์เดียว
boxdb download 2026-07-08 /root/restore          # ทุกไฟล์ในโฟลเดอร์วันนั้น
```

- ปลายทางต้องเป็นโฟลเดอร์ ถ้ายังไม่มีจะสร้างให้อัตโนมัติ
- ไฟล์เดิมจะไม่ถูกเขียนทับ — ถ้าชื่อซ้ำจะเติมเลขก่อนนามสกุลแทน:
  `db1.pg` → `db1 (1).pg` (นามสกุลซ้อนถูกจัดการให้ถูกต้อง:
  `backup.tar.gz` → `backup (1).tar.gz`)
- ระหว่างโหลดไฟล์จะใช้ชื่อชั่วคราว `.part` แล้วเปลี่ยนเป็นชื่อจริง
  เมื่อสมบูรณ์เท่านั้น — ไฟล์ที่ชื่อถูกต้องจึงการันตีว่าครบเสมอ

## การแจ้งเตือนทางอีเมล

หลัง `boxdb upload` ทุกครั้ง (ทั้งรันมือและรันตาม schedule) จะส่งอีเมล
สรุปผลผ่าน email API ของทีม เมื่อตั้งค่าครบทั้งสองอย่างด้านล่าง —
ถ้าขาดตัวใดตัวหนึ่งจะไม่ส่งแจ้งเตือน:

```sh
boxdb config --notify-to a@x.com,b@y.com   # ผู้รับ (หลายคนคั่นด้วย comma)
boxdb config --notify-token <token>        # token ของ email API
boxdb test-email                           # ทดสอบส่งอีเมลตัวอย่าง
```

- การอัพโหลดที่**ล้มเหลว**ก็ส่งอีเมลด้วย พร้อมสาเหตุในเนื้อหา —
  backup พังจะไม่เงียบหาย
- ปัญหาการส่งอีเมลไม่มีผลต่อผลลัพธ์ของ upload — แค่พิมพ์ warning
- Token ถูกเก็บแบบ**เข้ารหัส** (AES-256-GCM กุญแจ derive จาก
  `/etc/machine-id` ของเครื่อง): เปิด `config.json` อ่านจะไม่เห็นค่าจริง
  และ copy ไฟล์ไปเครื่องอื่นก็ถอดไม่ได้ — ต้องตั้ง token ใหม่ทีละเครื่อง
  รวมถึงหลังลง OS ใหม่
